...
All commands run from repo root unless noted. The Angular workspace holds three projects: the library (`forty-cdk`), a small dev-only application (`forty-cdk-harness`) used by the Playwright E2E suite, and the documentation site (`forty-cdk-docs`) that GitHub Pages deploys on every push to `main` — `pnpm build` and `pnpm test` are pinned to the library project so neither app ships, and the site's own suite runs under `pnpm test:docs`.
...
- **Workspace layout.** Single Angular CLI workspace, `projectType: library`. The library lives at `projects/forty-cdk/`: `src/public-api.ts` is the root entry point ng-packagr builds from (`ng-package.json`) and it is **`export {}` on purpose** — nothing is importable from the bare `forty-cdk` package name, so never add an export there ([#1590](https://github.com/tutkli/forty-cdk/issues/1590); the policy and the three rejected repairs are in `.claude/rules/conventions.md`). Everything a consumer imports is a **secondary entry point**: each primitive at `projects/forty-cdk/<primitive>/` (own `ng-package.json` pointing at `src/public-api.ts`, imported as `forty-cdk/<primitive>`), the blessed cross-primitive contracts from `forty-cdk/shared`, and the cross-primitive shared code in the `forty-cdk/core` entry point at `projects/forty-cdk/core/src/` (see Tree-shakability below); `src/lib/` now holds only the cross-cutting integration specs — the SSR smoke suite plus the contracts that span primitives instead of belonging to any single one (`ls projects/forty-cdk/src/lib/` is the inventory; a hand-copied list here rots) — and `src/test-utils/` the test helpers; the library's own `package.json` declares `"sideEffects": false` for tree-shaking and pins `@angular/{common,core}` as peers — keep both invariants.
...
- **Tree-shakability is a first-class constraint.** Avoid cross-primitive imports. Isolation is structural first: each primitive ships as its own **secondary entry point** (`forty-cdk/disclosure`, etc.) with its own FESM, so a consumer importing `ForDisclosure` gets a bundle their tooling never even sees `ForAccordion` in. `"sideEffects": false` + standalone directives then do the work _inside_ one entry point, where a root the consumer never imports — a variant sharing its base's entry, like `ForDateRangePicker` in `forty-cdk/date-picker` — drops out too. Both halves matter: the empty root barrel means there is no single barrel a tree-shaker could be asked to prune the whole library down from.
- **Optional peers must never be imported by value from an entry point that consumers who lack that peer will load.** Each entry point ships as a single FESM, and a consumer's bundler resolves every top-level import of that file before tree-shaking — an uninstalled optional peer breaks the build even for consumers who never touch the dependent primitive. The sanctioned shapes: (a) **type-only imports** for contract interfaces (`@angular/forms/signals` — erased at compile time, so the optional peer is genuinely optional); (b) a **dedicated secondary entry point** when consumer-facing values of the dependency cross the API (`forty-cdk/internationalized-date` holds the `@internationalized/date` adapters; only consumers importing that entry point need the peer, and it must stay a peer because consumers construct `CalendarDate` values themselves — a bundled copy would break `instanceof`); (c) a **regular `dependency`** (declared in `ng-package.json` `allowedNonPeerDependencies`) when the dependency is internal-only and nothing of it crosses the public API by value (`@floating-ui/dom` — auto-installed, tree-shaken out of non-overlay bundles). Inside a secondary entry point, import the core via the `forty-cdk/core` specifier, never by relative path.
...
**Never propagate state inside `effect()`.** Writing to a signal from an `effect` to derive other state is an anti-pattern (implicit cycles, double change-detection, ordering bugs). `effect()` is for **side effects** only (DOM imperative calls, non-signal subscriptions, logging, focus moves that can't be host bindings). Use `computed()` (pure derivation), `linkedSignal()` (writable state derived from a source), `resource()` / `httpResource()` (async), or `toSignal()` / `toObservable()` (RxJS bridge) instead. The two shapes that genuinely cannot be derived away each carry a canonical, lint-enforced marker on the `effect(` above them: `@sanctioned-effect(<invariant>)` for a write, and `@sanctioned-pull(<store>)` for a read that exists only to run a lazy fold over a transient source (whose tracked set must not widen a write sharing the effect — the lint bans the pair outright rather than judging the overlap). **Nor is `effect()` a validation channel:** a throw inside one reaches the application `ErrorHandler` with the scheduler as its stack and re-throws on every run, so an assertion goes at its point of use — **every** call the invalid state degrades, not just the first — behind an `isDevMode()`-gated `assert*` helper, and the lint fails an effect whose whole body is a `throw` / `assert*` call. The point of use is always attributable and one-shot; it is additionally catchable by the consumer only where it is a render-time derivation, since an event handler routes to the same `ErrorHandler`. The rule covers throws, not `fortyWarn`: an effect whose job is to warn is allowed when it is created only under `isDevMode()` and warns at most once per entry into the invalid state, and a warning about a registration count must read the settled count from one, since a structural swap transiently double-registers. Full rationale and the primitive-picker list → `.claude/rules/conventions.md`.
...
5. Tests in parallel — behavior and a11y (roles + aria + keyboard + focus), each under a title that names what it verifies. Do **not** add a per-primitive zoneless case: `renderHost` configures every fixture with `provideZonelessChangeDetection()`, so the whole suite already runs zoneless and the invariant is asserted once in `projects/forty-cdk/src/test-utils/render.spec.ts` ([#1595](https://github.com/tutkli/forty-cdk/issues/1595)). A spec that bypasses `renderHost` and builds its own `TestBed` must provide zoneless itself — that is the rule that still applies.
6. Add an SSR smoke fixture for the primitive in `projects/forty-cdk/src/lib/ssr/fixtures/` and register it in that folder's `registry.ts` — the suite is sweep-first, so one entry puts the primitive in every parameterised case, and no new hand-written `it` is owed. Declare the server-side ARIA pairings the primitive resolves (`markup`), and flag an overlay / `afterNextRender`-gated compose `open: true` so the `<body>` / listener / timer / position sweeps see it. This is mandatory: the library advertises SSR-safe primitives, and a primitive with no server-side assertion can ship a `document` / `window` access that only fails under Angular Universal. See `.claude/rules/testing.md` for the SSR coverage contract.
7. A `README.md` inside the primitive folder with a minimal styleless usage example. It opens with the frontmatter block the documentation site's registry is derived from (`title`, `group`, `archetype`, optional `apgUrl`) and a lede paragraph the site reads as the description, then carries the sections its declared archetype requires — the contract is enforced on every build, and both halves of it are in [docs/documentation-site-page-template.md](docs/documentation-site-page-template.md) ([#1808](https://github.com/tutkli/forty-cdk/issues/1808)). Its `## Examples` opens with a caption paragraph and then the **generated** hero fence: write the caption, run `pnpm gen:hero-fences`, and leave that fence alone afterwards — `pnpm check:hero-fences` fails a hand-edit ([#1934](https://github.com/tutkli/forty-cdk/issues/1934)).
8. Verify tree-shaking: the primitive imports cleanly in isolation.

## TypeScript expectations

Root `tsconfig.json` already enforces `strict`, `noImplicitOverride`, `noPropertyAccessFromIndexSignature`, `noImplicitReturns`, `noFallthroughCasesInSwitch`, plus Angular's `strictTemplates` / `strictInjectionParameters` / `strictInputAccessModifiers`. No `any` — use `unknown` and narrow. JSDoc on every public input/output/method/signal a consumer meets in IntelliSense, and it states the **contract, not the mechanism**: what the name and type already say needs no sentence, and how the piece works internally never belongs there. An options interface whose members are documented needs no header, and a symbol exported only for the internal tier is out of scope. Never build a developer-facing message by hand: every error and warning goes through `fortyError` / `fortyWarn` (or the `orphanContextError` / `unresolvedRootError` shape helpers) from `forty-cdk/core`, each carrying a stable `FORCDK-<AREA>-<NNN>` code that derives the `[forty-cdk/<entryPoint>]` prefix — the scheme, the `Cause` / `Fix` rules and the derived guard are in `.claude/rules/conventions.md`.
