...
- The defaults shape lives in `<primitive>/<primitive>-defaults.ts`, and is exported as a named interface following `For<Primitive>Defaults` (e.g. `ForToastDefaults`, `ForDialogDefaults`). The presence of `<primitive>-defaults.ts` is enforced by the `defaults-guard` job in `.github/workflows/ci.yml` — adding a primitive without it fails CI.
