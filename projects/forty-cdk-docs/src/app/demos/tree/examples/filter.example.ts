...
function buildAncestorsMap(roots: readonly TreeNodeData[]): Map<string, readonly string[]> {
  const map = new Map<string, readonly string[]>();
...
function collectMatchIds(roots: readonly TreeNodeData[], query: string): Set<string> {
...
function filterNodes(roots: readonly TreeNodeData[], query: string): readonly TreeNodeData[] {
  const needle = query.trim().toLowerCase();
  if (!needle) {
    return roots;
  }
...
@Component({
  ...
  styles: `
    ...
    .tree-mark {
      background: color-mix(in srgb, var(--ex-warning, #a5651a) 32%, transparent);
      color: inherit;
      border-radius: 3px;
      padding: 0 1px;
    }
