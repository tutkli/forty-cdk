...
@Component({
  ...
  template: `
    <playground-demo
      title="Avatar"
      summary="An image with a graceful fallback. The directive tracks the four-step load lifecycle (idle → loading → loaded → error) on data-status; the fallback is held back for fallbackDelayMs so a fast load never flashes initials."
    >
      <div demo>
