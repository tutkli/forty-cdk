/**
 * Exact public names of every `ForRadioGroup` input, its models included. Spread it into the
 ...
 * ```
 */
export const FOR_RADIO_GROUP_HOST_DIRECTIVE_INPUTS = [
  'value',
  'dir',
  'dirty',
  'disabled',
...
/**
 * Exact public names of every `ForRadioGroup` output, the Signal Forms `touch` output
 * included. Spread it into the `outputs` array of the same `hostDirectives` entry as
 * {@link FOR_RADIO_GROUP_HOST_DIRECTIVE_INPUTS}.
 */
export const FOR_RADIO_GROUP_HOST_DIRECTIVE_OUTPUTS = [
  'valueChange',
  'touchedChange',
  'touch',
] as const;
