...
describe('ForMeter', () => {
  describe('ARIA + reflection', () => {
    it('exposes role="meter" with valuemin / valuemax / valuenow', () => {
    ...
    it('clamps values outside [min, max] in the reflected aria-valuenow', () => {
      const { fixture, query, flush } = renderHost(MeterHost);
  ...
  describe('quality algorithm (HTML5 §<meter>)', () => {
    it('treats whole range as optimum when low/high/optimum are unset', () => {
      const { fixture, query, flush } = renderHost(MeterHost);
    ...
    it('optimum in middle: in [low, high] is optimum, outside is sub-optimum', () => {
      const { fixture, query, flush } = renderHost(MeterHost);
    ...
    it('optimum below low: below low = optimum, in [low,high] = sub-optimum, above high = even-less-good', () => {
      const { fixture, query, flush } = renderHost(MeterHost);
    ...
    it('optimum above high: above high = optimum, in [low,high] = sub-optimum, below low = even-less-good', () => {
      const { fixture, query, flush } = renderHost(MeterHost);
      fixture.componentInstance.low.set(40);
      fixture.componentInstance.high.set(80);
  ...
  describe('indicator', () => {
    it('reflects data-quality and CSS custom property on the indicator', () => {
      const { fixture, query, flush } = renderHost(MeterHost);
      fixture.componentInstance.value.set(40);
      flush();
