...
| `side` / `align` / `alignOffset` / `avoidCollisions` / `arrowPadding` / `sticky` / `hideWhenDetached` / `clipUntilPositioned` | `input<...>`             | Forwarded to the multiplexed `[forMenuContent]` when this trigger's menu is the one open. Same surface and same defaults as `[forDropdownMenu]`; `side` / `align` default from `provideForMenubarDefaults`.<br>**Default:** `'bottom'` / `'start'` / `0` / `true` / `0` / `'partial'` / `false` / `true` |
