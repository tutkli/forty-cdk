...
| `autoFocusOnOpen`    | `output<VetoableEvent>`                                   | Output. Just before focus moves to the first / last enabled item on mount (including a hover-switch remount).<br>**Default:** —                                                                            |
| `autoFocusOnClose`   | `output<VetoableEvent>`                                   | Output. Just before focus returns to the trigger on unmount. Not fired for Tab / outside-interaction closes, which already moved focus.<br>**Default:** —                                                  |
...
| `side` / `align` / `alignOffset` / `avoidCollisions` / `fallbackAxisSideDirection` / `arrowPadding` / `sticky` / `hideWhenDetached` / `clipUntilPositioned` | `input<...>`             | Forwarded to the multiplexed `[forMenuContent]` when this trigger's menu is the one open. Same surface and same defaults as `[forDropdownMenu]`.<br>**Default:** `'bottom'` / `'start'` / `0` / `true` / `'none'` / `0` / `'partial'` / `false` / `true` |
| `sideOffset` / `collisionPadding`                                                                                                                           | `input<number>`          | Gap (px) along the main axis / viewport collision padding (px). Defaults from `provideForMenubarDefaults`.<br>**Default:** `4` / `8`                                                                                                                     |
...
- **Hover-switch relocates keyboard focus.** Switching menus by hover is a full close/open cycle: the outgoing menu unmounts, so the focus it held has to be relocated or it would fall to `<body>`. Focus therefore lands on the incoming menu's first enabled item — without `data-highlighted`, so a mouse-opened menu never reads as preselected — and Arrow keys, typeahead and Escape keep working from there. The move passes `preventScroll` and only scrolls the item into view with `block: 'nearest'`, so sweeping the pointer across the bar never jumps the page. This is deliberately different from `[forMenuSub]`, whose hover-open never moves focus: a submenu opens _alongside_ the menu that holds focus, while a menubar hover-switch destroys it.
