...
- Submenus use `placement="right-start"` by default in LTR and `"left-start"` in RTL — set `[dir]="'rtl'"` on the top-level `[forDropdownMenu]` / `[forContextMenu]` and every nested `[forMenuSub]` inherits it (and flips `placement`, ArrowLeft/Right semantics, etc.). Override per-submenu with `[dir]` or `[placement]` if a specific submenu needs to render against the opposite direction.
