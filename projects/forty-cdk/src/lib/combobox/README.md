...
| `ForComboboxChips` | `[forComboboxChips]` | *(multi only)* Wrapper around the chips + the input. `role="group"`. |
| `ForComboboxChip` | `[forComboboxChip]` | *(multi only)* One chip per entry in `value()`. `value: required<string>`. |
| `ForComboboxChipRemove` | `[forComboboxChipRemove]` | *(multi only)* Remove `<button>` inside a chip with auto-generated `aria-label`. |
