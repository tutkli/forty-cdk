...
export class OverlayManagerCore<TEntry extends OverlayManagerEntry> {
  ...
  /**
   * @internal Generates the next per-instance id and a paired idempotent
  ...
  /**
   * @internal Drives the exit animation on the overlay root and, in lockstep,
  ...
  /**
   * @internal Builds a parent-cached per-component `Injector`. The returned
  ...
  /**
   * @internal Registers a fully-built entry, ensures the outlet exists, and
