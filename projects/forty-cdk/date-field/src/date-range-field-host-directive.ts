/**
 * Exact public names of every `ForDateRangeField` input, its model included. Spread it into
 ...
 *   ],
 * })
 * export class MyDateRangeField {}
 * ```
 */
export const FOR_DATE_RANGE_FIELD_HOST_DIRECTIVE_INPUTS = [
  'value',
  'ariaLabel',
  'dir',
  'dirty',
...
/**
 * Exact public names of every `ForDateRangeField` output, the Signal Forms `touch` output
 * included. Spread it into the `outputs` array of the same `hostDirectives` entry as
 * {@link FOR_DATE_RANGE_FIELD_HOST_DIRECTIVE_INPUTS}.
 */
export const FOR_DATE_RANGE_FIELD_HOST_DIRECTIVE_OUTPUTS = [
  'valueChange',
  'touchedChange',
  'touch',
] as const;
