    ...
    >
      @for (s of field.segments(); track s.id) {
        @if (s.isLiteral) {
          ...
          <span forDateFieldSegment [segment]="s.type!">{{ s.text }}</span>
        }
      }
    </div>
  `,
})
class DateFieldFormControlHost {
  readonly value = signal<Date | null>(null);
...
describe('ForDateField', () => {
  assertFormControlContract(
    () => {
      const r = renderHost(DateFieldFormControlHost);
      const result: FormControlMountResult = {
        control: r.query<HTMLElement>('[forDateField]')!,
        flush: r.flush,
        setFlag: (flag, value) => {
          switch (flag) {
            case 'readonly':
  ...
  describe('Signal Forms via [formField]', () => {
    ...
    it('flows schema-driven required into data-required (role="group" has no aria-required)', async () => {
      const r = renderHost(FormHost);
      await flush(r.fixture);
      const field = r.query('[forDateField]')!;
  ...
  describe('date-time (granularity > day)', () => {
    ...
    it('surfaces the time-capable-adapter requirement (not swallowed) under a day-only adapter (#590 F2)', () => {
      ...
      expect(() => renderHost(DayOnlyHost)).toThrow(
        /\[forty-cdk\/date-field\] FORCDK-CORE-003: ForDateField requires a time-capable DateAdapter/,
      );
    });
  });
