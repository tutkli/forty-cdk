...
describe('ForDateRangeField', () => {
  ...
  describe('disabled & readonly', () => {
    ...
    it('read-only field reflects data-readonly on the groups, aria-readonly on the segments', async () => {
      ...
      expect(seg(r, 'start', 'day').getAttribute('aria-readonly')).toBe('true');
      await key(r, 'start', 'day', 'ArrowUp');
  ...
  describe('Signal Forms via [formField]', () => {
    ...
    it('flows schema-driven required into data-required (role="group" has no aria-required)', async () => {
      ...
      await flush(r.fixture);
      const field = r.query('[forDateRangeField]')!;
  ...
  describe('date-time range (granularity > day)', () => {
    ...
    it('surfaces the time-capable-adapter requirement under a day-only adapter', () => {
      ...
      expect(() => renderHost(DayOnlyHost)).toThrow(
        /\[forty-cdk\/date-field\] FORCDK-CORE-003: ForDateRangeField requires a time-capable DateAdapter/,
      );
    });
  });
