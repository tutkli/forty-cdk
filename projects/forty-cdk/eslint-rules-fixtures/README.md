...
(the eight test-isolation rules in the `@forty-cdk-test-isolation-rules` block,
...
- `no-bare-detect-changes.fixture.ts` — 7 errors (five `detectChanges()` calls followed by an assertion on positioner output — inline `translate`, the anti-flash `clip-path` baseline, `data-side`, a positioner-written `--for-*` property, and the aliased read that proves the whole window is scanned rather than only the `expect(` lines — plus a bare `@sanctioned-sync-render` reported as malformed, and the #1606 anchoring case where a JSDoc block quoting the marker licenses nothing below it. Allowed: the well-formed marker, a window that ends at an `await flushPositioning(…)`, the host-bound `--for-carousel-offset` the mark list deliberately excludes, a `data-state` assertion, and a setup render with no assertion at all). The cases are one per exported function because the marker is matched by proximity — six lines above the call — so packing them into one body would let one case's marker license the next case's call.
